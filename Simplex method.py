def simplex_method():
    ...
    x = int(input("Enter number of rows of coefficients of constraints: "))
    z = int(input("Enter number of columns of coefficients of constraints: "))
    a = []
    for i in range(x):
        for j in range(z):
            a.append(int(input(f"Enter coefficients of constraints ({i},{j}): ")))            
    A1 = np.array(a)
    A = np.array_split(A1,2)
    
    # b will contain the amount of resources
    no_resources = int(input("Enter number of resources: "))
    b1 = []
    for i in range(no_resources):
        b1.append(int(input(f"Enter amount of resources ({i}): ")))
     
    b = np.array(b1)
    
    # c will contain coefficients of objective function Z
    no_objective_function = int(input("Enter number of coefficients of objective functions: "))
    c1 = []
    for i in range(no_objective_function):
        c1.append(int(input(f"Enter coefficients of objective function ({i}): ")))
    c = np.array(c1)

    # B will contain the basic variables that make identity matrix
    cb = np.array(c[3])
    no_basic_variables = int(input("Enter number of basic variables: "))
    B1 = []
    for i in range(no_basic_variables):
        B1.append(int(input(f"Enter basic variables ({i}): ")))
    basic_variables = np.array(B1)
    B = np.array_split(B1,2)
    ...
    MIN = int(input("Minimization(1),Maximization(0): "))
    ...
    print(basis)
    corner_point1 = int(table[0][2])
    corner_point2 = int(table[1][2])
    print("Corner points: ", end=" ")
    print(f"({corner_point1},{corner_point2})")
